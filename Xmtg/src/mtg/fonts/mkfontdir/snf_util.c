/*copyright	"%c%"
#ident	"@(#)r3fontutil:mkfontdir/snf_util.c	1.2"
/***********************************************************
Copyright 1988 by Digital Equipment Corporation, Maynard, Massachusetts,
and the Massachusetts Institute of Technology, Cambridge, Massachusetts.

                        All Rights Reserved

******************************************************************/

/* $XConsortium: snf_util.c,v 1.7 88/10/10 19:34:56 rws Exp $ */

#include <stdio.h>
#include <X11/Xos.h>
#include <sys/stat.h>

#include <X11/X.h>
#include <X11/Xproto.h>
#include <X11/Xmd.h>

#include "fontstruct.h"
#include "snfstruct.h"
#include "bdftosnf.h"

#include "fontdir.h"

/*
** Function:	GetSNFFontName
** Description:
**
** Extract a font's name from it's FONT property in the .snf file.
**
** Modified by:	Karl Larson
** Date:	8/24/89
** Modification:
**
** On the 3B2, swap the bytes of the font and property information
** before processing them.  This is because the .snf files are in
** 6386/630 X Server format.
*/
void GetSNFFontName( fp, font_name)
    FILE *fp;
    char *font_name;
{
    FontInfoRec	fi;
    FontPropPtr	pffp;
    char	*propspace;
    int		bytestoread;
    int		i;
    char	*strings;
    struct stat	stats;
    int		c;

#ifdef u3b2
    void	SwapSelectedFontInfo(), SwapPropInfo();
#endif

    bytestoread = sizeof(FontInfoRec);
    if ( fread((char *)&fi, 1, bytestoread, fp) != bytestoread) {
	font_name[0] = NUL;
        return;
    }

#ifdef u3b2
    /*
     * Swap the font information.
     */
    SwapSelectedFontInfo(&fi);
#endif
    if (fi.version1 != FONT_FILE_VERSION || fi.version2 != FONT_FILE_VERSION) {
	font_name[0] = NUL;
	return;
    }

    /* skip what we find unintersting */
    i = BYTESOFCHARINFO(&fi) + BYTESOFGLYPHINFO(&fi);
    fstat(fileno(fp), &stats);
    if ((stats.st_mode & S_IFMT) == S_IFREG)
	fseek(fp, i, 1);
    else
	while (i-- && ((c = getc(fp)) != EOF))
	    ;

    /* now read properties */
    bytestoread = BYTESOFPROPINFO(&fi) + BYTESOFSTRINGINFO(&fi);
    propspace = (char *) malloc(bytestoread);

    if ( fread(propspace, 1,	bytestoread, fp) != bytestoread) {
	free(propspace);
	font_name[0] = NUL;
        return;
    }

    pffp = (FontPropPtr)propspace;
#ifdef u3b2
    /*
     * Swap the properties.
     */
    SwapPropInfo(pffp, fi.nProps);
#endif
    strings = propspace + BYTESOFPROPINFO(&fi);
    for (i = 0; i < fi.nProps; i++, pffp++) {
	if (strcmp(&strings[pffp->name], "FONT") == 0) {
	    if (pffp->indirect)	/* right type for name */
		strcpy(font_name, &strings[pffp->value]);
	    break;
	}
    }
    free(propspace);
}

#ifdef u3b2

unsigned char _reverse_byte[0x100] = {
	
};

/*
 *	Invert bit order within each BYTE of an array.
 */
void
bitorderinvert(buf, nbytes)
    register unsigned char *buf;
    register int nbytes;
{
    register unsigned char *rev = _reverse_byte;
    for (; --nbytes >= 0; buf++)
	*buf = rev[*buf];
}

/*
** Function:	SwapSelectedFontInfo
** Author:	Karl Larson
** Date:	8/24/89
** Input:	pfi - Pointer to font information record.
** Output:	None
** Description:
**
** Swap needed fields in font info record to put in 630 format on
** 3B2.
*/
void
SwapSelectedFontInfo(pfi)
    register FontInfoPtr pfi;
{
    register unsigned long *bitptr;

    void bitorderinvert();

    /*
     * swap the bytes of each field.  Only swap fields we need.
     */

    pfi->version1 = lswapl(pfi->version1);
    pfi->firstCol = lswapl(pfi->firstCol);
    pfi->lastCol = lswapl(pfi->lastCol);
    pfi->firstRow = lswapl(pfi->firstRow);
    pfi->lastRow = lswapl(pfi->lastRow);
    pfi->nProps = lswapl(pfi->nProps);
    pfi->lenStrings = lswapl(pfi->lenStrings);
    pfi->version2 = lswapl(pfi->version2);
    /* Swap the maxbounds.byteoffset field */
    bitptr = (unsigned long *)(&pfi->maxbounds.metrics + 1);
    *bitptr = lswapl(*bitptr & 0xFFFFFF00) << 8;
    return;
}

/*
** Function:	SwapPropInfo
** Author:	Karl Larson
** Date:	8/23/89
** Input:	pfp - Pointer to font properties.
** Output:	None
** Description:
**
** Swap all the fields of the property records to put into
** 630 format on the 3B2.
*/
void
SwapPropInfo(pfp, numprops)
    FontPropPtr pfp;
    int numprops;
{

    while (numprops--)
    {
	pfp->name = lswapl(pfp->name);
	pfp->value = lswapl(pfp->value);
	pfp->indirect = lswapl(pfp->indirect); 
	pfp++;
    }
    return;
}
#endif
