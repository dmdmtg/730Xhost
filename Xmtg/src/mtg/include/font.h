#ifndef	NOIDENT
#ident	"@(#)font.h	2.1.2.1"
#endif
/*
 font.h (C header file)
 ...
 * for 2D char sets
 */
#ifdef USE_SHORT_INTS
#define n2dChars(pfi)	(((pfi)->lastCol - (pfi)->firstCol + 1) * \
			 ((pfi)->lastRow - (pfi)->firstRow + 1))
#else
#define n2dChars(pfi)	( ( ((unsigned long)(pfi)->lastCol) \
			    - ((unsigned long)(pfi)->firstCol) + 1) * \
			  ( ((unsigned long)(pfi)->lastRow) \
			    - ((unsigned long)(pfi)->firstRow) + 1) )
#endif
...
/* Lines 88 - 105 defines are not in R3.  May need to change some things
 * in ddx!
 */
/*
...
/* #define	BYTESOFGLYPHINFO(pfi)	(((pfi)->maxbounds.byteOffset+1) & ~0x1) */
#define	BYTESOFGLYPHINFO(pfi)	((unsigned long)(((unsigned long)((pfi)->maxbounds.byteOffset))+1 & ~0x1))
...
typedef unsigned long DrawDirection;
